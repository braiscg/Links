import instagram from "../assets/instagram.png";
import youtube from "../assets/youtube.jpeg";
import spotify from "../assets/spotify.png";
import telegram from "../assets/telegram.png";
import twitter from "../assets/twitter.png";
import linkedin from "../assets/linkedin.png";
import consupermiso from "../assets/consupermiso.png";
import salylaurel from "../assets/salylaurel.png";

const items = [
  {
    title: "Linkedin",
    subtitle: "I talk about corporate innovation, business, new ventures, entrepreneurship and my path enjoying my professional life.",
    image: linkedin,
    link: "https://www.linkedin.com/in/braiscomesana/", //your personal website or portfolio  link
  },

  {
    title: "Twitter",
    subtitle: "@braiscg | Don't forget to follow me. I'm trying to build an audience about spanish corporate ecosystem 😉",
    image: twitter,
    link: "https://twitter.com/braiscg", // twitter profile link
  },
  {
    title: "Innovation takes Guts - Spotify",
    subtitle:
      "Listen to the interviews of our podcast about Corporate Innovation",
    image: spotify,
    link: "https://open.spotify.com/show/34cEEACZyNo4mJTqgLYuFs", // Dribbble profile link
  },

  {
    title: "Sal y Laurel",
    subtitle: "Tienda online de pescado y marisco de Galicia",
    image: salylaurel,
    link: "https://salylaurel.es/", //Telegram Pofile
  },

  {
    title: "Innovation Takes Guts - YouTube",
    subtitle: "I host a podcast about corporates building new ventures",
    image: youtube,
    link: "https://www.youtube.com/channel/UC1mqX7sytX3JX0mPi16lbYg", //youtube channel link
  },
  {
    title: "Instagram",
    subtitle: "@braiscg| Pictures and stories about my personal life with friends and my love.",
    image: instagram,
    link: "https://instagram.com/braiscg", //instagram profile link
  },
  {
    title: "Consupermiso",
    subtitle: "Cashback platform of Grupo VIKO",
    image: consupermiso,
    link: "https://www.consupermiso.com/signup?referer=5b97ccc9e2e03e68508b4c74", //Telegram Pofile
  },
  {
    title: "Telegram",
    subtitle: "@braiscg | Chat with me instantly, Don't hesitate! ",
    image: telegram,
    link: "https://telegram.me/braiscg", //Telegram Pofile
  },
];

export default items;
